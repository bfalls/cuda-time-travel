...
bool parse_checkpoint_range(const char* text, uint32_t& begin, uint32_t& end) {
    if (!text || !*text) {
        return false;
    }
...
void print_usage() {
    std::printf("Usage:\\n");
    std::printf("  tt verify --manifest <path> [--checkpoints <start-end>] [--regions <list>] [--out <report.json>] [--trace-annotate] [--trace-out <path>] [--continue] [--localize] [--tamper <checkpoint,region,offset>]\\n");
}
...
int main() {
    ...
    for (int i = 2; i < __argc; ++i) {
        if (std::strcmp(__argv[i], "--trace-annotate") == 0) {
        ...
        } else if (std::strcmp(__argv[i], "--checkpoints") == 0 && i + 1 < __argc) {
            checkpoint_range_set = parse_checkpoint_range(__argv[++i], checkpoint_begin, checkpoint_end);
        } else if (std::strncmp(__argv[i], "--checkpoints=", 9) == 0) {
            checkpoint_range_set = parse_checkpoint_range(__argv[i] + 9, checkpoint_begin, checkpoint_end);
            ...
            std::string checkpoint_text(value, first - value);
            std::string region_text(first + 1, second - first - 1);
            std::string offset_text(second + 1);
            if (!parse_u32(checkpoint_text.c_str(), tamper_checkpoint) ||
    ...
    options.checkpoint_range_set = checkpoint_range_set;
    options.checkpoint_begin = checkpoint_begin;
    options.checkpoint_end = checkpoint_end;
    options.region_ids = regions;
    if (tamper_enabled) {
        options.tamper.enabled = true;
        options.tamper.checkpoint_id = tamper_checkpoint;
